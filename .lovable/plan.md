...
- `src/pages/AdminPage.tsx` :
  ...
  - Petit composant local `LocationPicker` réutilisé dans `AddEventTab` et dans le bloc d'édition de `EventsTab`. Il appelle lui-même `useAllLocations()` (sans condition) : l'appel existant ligne ~255 est limité aux onglets `locations`/`contributions`, la clé react-query étant partagée, un appel propre dans le picker suffit à alimenter la liste depuis les onglets sortie.
  - Le picker ne propose que les lieux `status === 'published'` (les brouillons sont filtrés) : les apps iOS/Android ne chargent le lieu lié qu'en `published`, un rattachement à un brouillon serait silencieusement invisible. Filtrage client sur `name`/`city`, ~8 résultats affichés.
  - `AddEventTab.handleAddEvent` : si un lieu est sélectionné, `address/lat/lng` proviennent du lieu et le bloc `geocodeAddress` est court-circuité ; `location_id` et `recurrence_label` sont ajoutés au payload d'insert.
  ...
  - `location_id` est un uuid : dans **les deux** formulaires (ajout et édition) la valeur vide part en `null`, via la convention déjà en place dans le fichier (`form.location_id || null`). Même traitement pour `recurrence_label` (trim puis `|| null`). Un `''` envoyé en base produirait une erreur `invalid input syntax for type uuid` au moment de sauver.
