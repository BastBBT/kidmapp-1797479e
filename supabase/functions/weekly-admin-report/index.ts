...
async function runReport() {
  ...
  console.log('weekly-admin-report enqueued', { periodLabel, totalContributions, totalProposals, activeUsers })
  return { periodLabel, totalContributions, totalProposals, activeUsers }
}
...
Deno.serve(async (_req) => {
  try {
    // Kick off the work in background so we can ACK quickly (avoids pg_cron 5s net.http_post timeout)
    // @ts-ignore
    EdgeRuntime.waitUntil(
      runReport().catch((e) => console.error('weekly-admin-report background error', e)),
    )
...
// Legacy entry kept for reference (no-op)
