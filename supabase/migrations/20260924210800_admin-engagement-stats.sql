...
BEGIN
  ...
  logs AS (
    SELECT l.template_name, l.status,
           ...
           l.message_id,
           row_number() OVER (
             PARTITION BY COALESCE(l.message_id, l.id::text)
  ...
  sends AS (
    SELECT t.tpl,
      jsonb_build_object(
        'emailSent',      count(*) FILTER (WHERE NOT l.is_push AND l.rn = 1 AND l.status = 'sent'),
        'emailFailed',    count(*) FILTER (WHERE NOT l.is_push AND l.rn = 1 AND l.status = 'failed'),
        'emailSuppressed',count(*) FILTER (WHERE NOT l.is_push AND l.rn = 1 AND l.status = 'suppressed'),
        'skippedNoMatch', count(*) FILTER (WHERE NOT l.is_push AND l.rn = 1 AND l.status = 'skipped_no_match'),
