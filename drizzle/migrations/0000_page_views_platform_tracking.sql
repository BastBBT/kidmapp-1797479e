...
CREATE INDEX IF NOT EXISTS page_views_platform_created_at_idx
  ON public.page_views (platform, created_at DESC);
  ...
  FROM (
    SELECT g.day::text AS day_key,
           jsonb_object_agg(pl.platform, jsonb_build_object(
             ...
             'uniques', COALESCE(s.uniques, 0))) AS plats
